...
def benchmark():
    ...
    parser.add_argument("--dense_profile", action="store_true",
                        help="Use dense profile for benchmarking")
    args = parser.parse_args()

    seerattn_sparsity_method = "none" if args.dense_profile else "threshold"
    model = SeerDecodingQwen2ForCausalLM.from_pretrained(
        ...
        seerattn_sparsity_method=seerattn_sparsity_method,
        load_gate=False,
    )
    ...
    for _ in tqdm(range(decode_len)):
        input_ids = torch.randint(0, 100, (batch_size, 1), dtype=torch.int64, device=device)
        attention_mask = torch.cat([attention_mask, torch.ones((attention_mask.size(0), 1), device=device)], dim=1)
        ts = time.perf_counter()
        outputs = model(
